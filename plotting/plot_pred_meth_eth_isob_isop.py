...
data = np.load("../training/methane_ethane_isob_isop/sorted_predictions_right_model.npz")

pred_ene_methane_traj = data["arr_0"]
true_ene_methane_traj = data["arr_1"]

pred_ene_ethane_traj = data["arr_2"]
true_ene_ethane_traj = data["arr_3"]

pred_ene_isobutane_traj = data["arr_4"]
true_ene_isobutane_traj = data["arr_5"]

pred_ene_isopentane_traj = data["arr_6"]
true_ene_isopentane_traj = data["arr_7"]
...
print("Ethane error for MAE: %s" % str(np.std(true_ene_ethane_traj-pred_ene_ethane_traj)))

x = list(range(len(pred_ene_ethane_traj)))
fig, ax = plt.subplots(figsize=(10,10))

subpos = [0.15,0.67,0.3,0.3] # Relative x,y and height, width
ax_font = {'size':'19'}
subax_font = {'size':'16'}
ax.scatter(x, true_ene_ethane_traj, label="True values", s=50)
ax.scatter(x, pred_ene_ethane_traj, label="Predictions", s=50)
ax.set_xlabel("Frame Number", labelpad=7, **ax_font)
ax.set_ylabel("Scaled energy (kJ/mol)", labelpad=5, **ax_font)
ax.tick_params(axis='x', labelsize=24)
ax.tick_params(axis='y', labelsize=24)
ax.legend(prop={'size':15})
subax1 = add_subplot_axes(ax, subpos)
subax1.scatter(true_ene_ethane_traj, pred_ene_ethane_traj, s=10, c=sns.color_palette()[2])
subax1.set_xlabel("True energy (kJ/mol)", labelpad=3, **subax_font)
subax1.set_ylabel("Predicted energy (kJ/mol)", labelpad=2, **subax_font)
subax1.tick_params(axis='x', labelsize=20)
subax1.tick_params(axis='y', labelsize=20)
plt.savefig("./ethane.png", dpi=200)
# plt.show()

# Isobutane trajectory

print("Isobutane error for MAE: %s" % str(np.std(true_ene_isobutane_traj-pred_ene_isobutane_traj)))

x = list(range(len(pred_ene_isobutane_traj)))
fig, ax = plt.subplots(figsize=(10,10))

subpos = [0.67,0.67,0.3,0.3] # Relative x,y and height, width
ax_font = {'size':'19'}
subax_font = {'size':'16'}
ax.scatter(x, true_ene_isobutane_traj, label="True values", s=50)
ax.scatter(x, pred_ene_isobutane_traj, label="Predictions", s=50)
ax.set_xlabel("Frame Number", labelpad=7, **ax_font)
ax.set_ylabel("Scaled energy (kJ/mol)", labelpad=5, **ax_font)
ax.tick_params(axis='x', labelsize=24)
ax.tick_params(axis='y', labelsize=24)
...
subax1 = add_subplot_axes(ax, subpos)
subax1.scatter(true_ene_isobutane_traj, pred_ene_isobutane_traj, s=10, c=sns.color_palette()[2])
subax1.set_xlabel("True energy (kJ/mol)", labelpad=3, **subax_font)
subax1.set_ylabel("Predicted energy (kJ/mol)", labelpad=2, **subax_font)
subax1.tick_params(axis='x', labelsize=20)
subax1.tick_params(axis='y', labelsize=20)
plt.savefig("./isobutane.png", dpi=200)
# plt.show()
